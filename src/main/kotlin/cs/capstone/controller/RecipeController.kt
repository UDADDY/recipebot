...
//) {
//    @GetMapping
//    fun getRecipeHistory(request: HttpServletRequest,): ResponseEntity<List<RecipeDraft>> {
//        val userId = request.currentUserIdOrNull()!!
//        val history: List<Recipe> = recipeService.getHistory(userId)
////        val responses = history.map { RecipeDraft(it) }
//
////        return ResponseEntity.ok(responses)
//    }
//
//    @GetMapping("/{recipeId}")
//    fun getRecipe(@PathVariable recipeId: Long): ResponseEntity<RecipeDraft> {
//        val response = recipeService.getRecieptById(recipeId)
//
////        return ResponseEntity.ok(RecipeDraft.fromEntity(response))
//    }
//
